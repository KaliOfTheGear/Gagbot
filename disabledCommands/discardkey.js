...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addUserOption((opt) => opt.setName("user").setDescription("Keys to who?").setRequired(false)),
		)
		.addSubcommand((subcommand) =>
			subcommand
				.setName("collar")
	...
	async execute(interaction) {
		...
		if (!restraint) {
			interaction.reply({ content: "Unknown restraint, blame <@458684324653301770>", flags: MessageFlags.Ephemeral });
			return;
		}
		...
		if (!getKeyholderFunction(lockedUser.id)) {
			if (lockedUser.id == interaction.user.id) {
				interaction.reply({ content: "You are not locked in that type of restraint", flags: MessageFlags.Ephemeral });
			} else {
				interaction.reply({ content: `${lockedUser} is not locked in that type of restraint`, flags: MessageFlags.Ephemeral });
			}
		...
		if (getHeavy(interaction.user.id)) {
			interaction.reply(`${interaction.user} tugs against ${their(interaction.user.id)} ${getHeavy(interaction.user.id).type}, trying to give ${their(interaction.user.id)} keys to ${lockedUser}'s ${restraint} to someone else, but it is futile!`);
			return;
		}

		if (getKeyholderFunction(lockedUser.id) != interaction.user.id) {
			interaction.reply({ content: `You do not have the key to ${lockedUser}'s ${restraint}`, flags: MessageFlags.Ephemeral });
			return;
		}

		if (lockedUser.id == interaction.user.id) {
			interaction.reply(`${interaction.user} discards the keys to ${their(lockedUser.id)} ${restraint}! Who knows when they'll be found.`);
			discardFunction(lockedUser.id, interaction.user.id);
		} else {
			interaction.reply(`${interaction.user} discards their keys to ${lockedUser}'s ${restraint}! Who knows when they'll be found.`);
			discardFunction(lockedUser.id, interaction.user.id);
		}
	},
};
